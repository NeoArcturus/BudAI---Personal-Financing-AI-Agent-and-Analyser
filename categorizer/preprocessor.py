...
class Preprocessor:
    def __init__(self, data_file):
        self.sentence_tf = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2")
        ...
        self.standard_columns = ["Date", "Amount", "Currency", "Description", "Payment Reference", "Target Name", "Transaction Type", "Transaction Details Type"]
    ...
    def standardize_columns(self):
        manual_map = {"Merchant": "Target Name", "Narrative": "Description", "Reference": "Payment Reference", "Debit Amount": "Amount", "Credit Amount": "Amount"}
        ...
        std_emb = self.sentence_tf.encode(self.standard_columns, convert_to_tensor=True)
        ...
        for m in all_matches:
            if m['score'] < 0.75 or m['original'] in used_original or m['standard'] in used_standard: continue
            rename_map[m['original']] = m['standard']
            used_original.add(m['original']); used_standard.add(m['standard'])
        self.df.rename(columns=rename_map, inplace=True)
        self.df = self.df[[c for c in self.df.columns if c in self.standard_columns]]
