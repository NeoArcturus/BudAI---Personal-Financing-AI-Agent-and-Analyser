...
@tool
def trigger_failsafe_sync(user_uuid: str) -> str:
    """Forces a manual sync to catch missed webhooks."""
    return f"Triggered failsafe sync for {user_uuid}." 

tools = []

class CloudFailsafeAgent:
    """
    Standalone LangGraph Agent with ChatOpenAI capabilities for CloudFailsafeAgent.
    """
    def __init__(self):
    ...
    def _build_graph(self):
        ...
        def evaluate_node(state: AgentState):
            logger.info("CloudFailsafeAgent evaluating state...")
            messages = list(state.get("messages", []))
            
            if not any(isinstance(m, SystemMessage) for m in messages):
                sys_msg = SystemMessage(
                    content=(
                        "### ROLE: Cloud Failsafe Specialist\n"
                        "You are the Cloud Failsafe Agent. Your job is to force a manual physical bank sync if webhooks fail.\n\n"
