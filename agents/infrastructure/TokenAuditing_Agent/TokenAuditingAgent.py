...
@tool
def audit_token_usage(system_id: str) -> str:
    """Audits LLM token usage across the system."""
    return f"Audited token usage for {system_id}." 

tools = []

class TokenAuditingAgent:
    """
    Standalone LangGraph Agent with ChatOpenAI capabilities for TokenAuditingAgent.
    """
    def __init__(self):
    ...
    def _build_graph(self):
        ...
        def evaluate_node(state: AgentState):
            logger.info("TokenAuditingAgent evaluating state...")
            messages = list(state.get("messages", []))
            
            if not any(isinstance(m, SystemMessage) for m in messages):
                sys_msg = SystemMessage(
                    content=(
                        "### ROLE: Token Auditing Specialist\n"
                        "You are the Token Auditing Agent. Your job is to monitor system-wide LLM token usage to prevent runaway compute costs.\n\n"
