...
@tool
def extract_receipt_data(document_id: str) -> str:
    """Extracts merchant, amount, and date from a receipt image/PDF."""
    return f"Extracted data from document {document_id}." 

tools = []

class DocumentProcessingAgent:
    """
    Standalone LangGraph Agent with ChatOpenAI capabilities for DocumentProcessingAgent.
    """
    def __init__(self):
    ...
    def _build_graph(self):
        ...
        def evaluate_node(state: AgentState):
            logger.info("DocumentProcessingAgent evaluating state...")
            ...
            if not any(isinstance(m, SystemMessage) for m in messages):
                sys_msg = SystemMessage(
                    content=(
                        ...
                        "You are the Document Processing Agent. Your job is to extract text and data from uploaded physical receipts or bills and match them to transactions.\n\n"
