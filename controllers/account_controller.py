...
async def get_accounts(current_user: User = Depends(get_current_user)):
    try:
        ...
        logger.error(
            f"Failed to fetch accounts for user {current_user.user_uuid}: {e}")
        raise HTTPException(
    ...
    try:
        ...
        if 'amount' in df.columns:
            df['amount'] = pd.to_numeric(
                df['amount'], errors='coerce').fillna(0.0)
        ...
        logger.error(
            f"Failed to fetch transactions for user {current_user.user_uuid}, account {account_id}: {e}")
        raise HTTPException(
...
async def revoke_connection(provider_id: str, current_user: User = Depends(get_current_user)):
    try:
        ...
        logger.warning(
            f"Failed to revoke connection for provider: {provider_id}")
        raise HTTPException(
            status_code=500, detail="Failed to revoke connection")
    except Exception as e:
        logger.error(
            f"Error revoking connection for user {current_user.user_uuid}, provider {provider_id}: {e}")
