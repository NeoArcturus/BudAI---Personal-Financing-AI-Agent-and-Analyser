...
@tool("call_document_processing_agent", description="Use this tool to delegate OCR tasks, receipt scanning, invoice extraction, and processing unstructured financial documents into structured data.")
async def call_document_processing_agent(
    query: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
    state: Annotated[BudAIState, InjectedState]
):
    """Delegate to the DocumentProcessing Agent."""
    try:
        user_uuid = state.get("user_uuid", "ea0e5c07-ab5b-4c14-9ad9-95a036b24637")
        
        from agents.infrastructure.DocumentProcessing_Agent.DocumentProcessingAgent import DocumentProcessingAgent
        ...
        logger.error(f"Error in DocumentProcessing worker: {e}")
        return Command(update={
            "messages": [
                ToolMessage(content=f"Error executing DocumentProcessing tool: {str(e)}", tool_call_id=tool_call_id)
            ]
        })
