...
@tool("call_goal_tracking_agent", description="Use this tool to delegate tracking progress towards savings targets, evaluating milestone achievements, and calculating the required funding rate to meet specific financial deadlines.")
async def call_goal_tracking_agent(
    query: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
    state: Annotated[BudAIState, InjectedState]
):
    """Delegate to the GoalTracking Agent."""
    try:
        user_uuid = state.get("user_uuid", "ea0e5c07-ab5b-4c14-9ad9-95a036b24637")
        
        from agents.core_financial.GoalTracking_Agent.GoalTrackingAgent import GoalTrackingAgent
        ...
        logger.error(f"Error in GoalTracking worker: {e}")
        return Command(update={
            "messages": [
                ToolMessage(content=f"Error executing GoalTracking tool: {str(e)}", tool_call_id=tool_call_id)
            ]
        })
