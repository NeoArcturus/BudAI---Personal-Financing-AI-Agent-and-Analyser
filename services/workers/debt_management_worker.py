...
@tool("call_debt_management_agent", description="Use this tool to delegate liability analysis, interest rate calculations, payoff velocity projections (Avalanche/Snowball methods), and debt reduction strategies.")
async def call_debt_management_agent(
    query: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
    state: Annotated[BudAIState, InjectedState]
):
    """Delegate to the DebtManagement Agent."""
    try:
        user_uuid = state.get("user_uuid", "ea0e5c07-ab5b-4c14-9ad9-95a036b24637")
        
        from agents.core_financial.DebtManagement_Agent.DebtManagementAgent import DebtManagementAgent
        ...
        logger.error(f"Error in DebtManagement worker: {e}")
        return Command(update={
            "messages": [
                ToolMessage(content=f"Error executing DebtManagement tool: {str(e)}", tool_call_id=tool_call_id)
            ]
        })
