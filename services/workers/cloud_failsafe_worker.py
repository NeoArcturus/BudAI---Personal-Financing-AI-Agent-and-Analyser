...
@tool("call_cloud_failsafe_agent", description="Use this tool to delegate cloud synchronization, backup operations, or verifying the disaster recovery status of the user's encrypted financial state.")
async def call_cloud_failsafe_agent(
    query: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
    state: Annotated[BudAIState, InjectedState]
):
    """Delegate to the CloudFailsafe Agent."""
    try:
        ...
        logger.error(f"Error in CloudFailsafe worker: {e}")
        return Command(update={
            "messages": [
                ToolMessage(content=f"Error executing CloudFailsafe tool: {str(e)}", tool_call_id=tool_call_id)
            ]
        })
