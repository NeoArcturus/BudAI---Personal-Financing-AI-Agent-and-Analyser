...
@tool("call_dynamic_interface_agent", description="Use this tool to delegate the generation and WebSocket pushing of dynamic UI elements, chart configurations, or interactive widgets directly to the frontend.")
async def call_dynamic_interface_agent(
    query: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
    state: Annotated[BudAIState, InjectedState]
):
    """Delegate to the DynamicInterface Agent."""
    try:
        user_uuid = state.get("user_uuid", "ea0e5c07-ab5b-4c14-9ad9-95a036b24637")
        
        from agents.execution.DynamicInterface_Agent.DynamicInterfaceAgent import DynamicInterfaceAgent
        agent = DynamicInterfaceAgent()
        ...
        logger.error(f"Error in DynamicInterface worker: {e}")
        return Command(update={
            "messages": [
                ToolMessage(content=f"Error executing DynamicInterface tool: {str(e)}", tool_call_id=tool_call_id)
            ]
        })
