...
@tool("call_token_auditing_agent", description="Use this tool to delegate the monitoring of LLM token consumption, auditing API usage limits, and optimizing prompt efficiency across the agent network.")
async def call_token_auditing_agent(
    query: str,
    config: RunnableConfig,
    tool_call_id: Annotated[str, InjectedToolCallId],
    state: Annotated[BudAIState, InjectedState]
):
    """Delegate to the TokenAuditing Agent."""
    try:
        ...
        logger.error(f"Error in TokenAuditing worker: {e}")
        return Command(update={
            "messages": [
                ToolMessage(content=f"Error executing TokenAuditing tool: {str(e)}", tool_call_id=tool_call_id)
            ]
        })
