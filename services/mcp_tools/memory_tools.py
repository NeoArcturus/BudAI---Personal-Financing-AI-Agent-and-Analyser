...
def search_financial_history_semantic(query: str = "", user_uuid: str = "") -> str:
    ...
    """
    logger.info(json.dumps({"message": f"Executing search_financial_history_semantic for user {user_uuid}", "status_code": 200}))
    try:
        mem = MemoryService()
        ...
        logger.error(json.dumps({"message": f"Error in search_financial_history_semantic: {e}", "status_code": 500}))
...
def get_seasonal_behavior_context(user_uuid: str = "") -> str:
    ...
    """
    logger.info(json.dumps({"message": f"Executing get_seasonal_behavior_context for user {user_uuid}", "status_code": 200}))
    try:
        mem = MemoryService()
        ...
        logger.error(json.dumps({"message": f"Error in get_seasonal_behavior_context: {e}", "status_code": 500}))
