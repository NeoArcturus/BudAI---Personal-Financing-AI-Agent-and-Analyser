...
class ForecasterAgent:
    def __init__(self, db_path=None):
        ...
        if _global_lstm is None:
            _global_lstm = ParameterLSTM().to(self.device)
            if os.path.exists(self.model_path):
                _global_lstm.load_state_dict(torch.load(
    ...
    def fetch_and_calculate_parameters(self, account_id, current_balance, user_uuid, lookback_days=60):
        ...
        df['Date'] = pd.to_datetime(df.get('date', df.get(
            'timestamp')), format='ISO8601', utc=True).dt.date
    ...
    def fetch_expense_parameters(self, account_id, user_uuid, lookback_days=60):
        ...
        recent_data = df.groupby('Date')['amount'].sum().reset_index(
    ...
    def detect_upcoming_expenses(self, user_uuid, account_id=None, days=30, df_input=None):
        ...
        df['Date'] = pd.to_datetime(
            df.get('date', df.get('timestamp')), format='ISO8601', utc=True)
        critical_categories = ["Bills & Utilities", "Housing",
                               "Entertainment", "Transfers & Investments", "Taxes"]
        df_out = df[(df['amount'] < 0) & (
            df['category'].isin(critical_categories))].copy()

        calendar, timeline = np.zeros(days + 1), []
        now = pd.Timestamp.utcnow()

        df_out['group_desc'] = df_out['description'].str.replace(r'\d+\.\d+\s?\w*', '', regex=True).str.strip()

        for (desc_key, cat), group in df_out.groupby(['group_desc', 'category']):
            if len(group) >= 2:
                group = group.sort_values('Date')
                deltas = group['Date'].diff().dt.days.dropna()
                median_delta = deltas.median()
                
                max_std = 5 if len(group) <= 5 else 8
                
                if 25 <= median_delta <= 35 and deltas.std() < max_std:
                    projected_amount = group['amount'].iloc[-1]
                    display_desc = group['description'].iloc[-1]
                    ...
                    while (next_date - now).days <= days:
                        ...
                        if 0 <= day_idx <= days:
                            calendar[day_idx] += projected_amount
                            timeline.append({'day': day_idx, 'date': next_date.strftime(
                                '%Y-%m-%d'), 'merchant': display_desc, 'category': cat, 'amount': round(float(projected_amount), 2)})
    ...
    def run_hybrid_simulation(self, account_id, S0, mu, user_uuid, days=60, paths=1000000, discipline_multiplier=1.0, drift_adjustment=0.0, stress_test_active=False, macro_environment="Stable"):
        ...
        if discipline_multiplier != 1.0 or drift_adjustment != 0.0 or stress_test_active or macro_environment != "Stable":
            if discipline_multiplier < 1.0:
                params['theta'], params['xi'] = params['theta'] * \
                    0.8, params['xi']*0.7
            ...
            if macro_environment == "Inflationary":
                params['theta'], mu = params['theta']*1.2, mu-0.005
            elif macro_environment == "Recession":
                params['lambda'], params['mu_J'] = params['lambda'] + \
                    0.2, params['mu_J']-0.1
            if stress_test_active:
                params['lambda'], params['mu_J'], params['sigma_j'] = 0.8, -0.4, 0.2
        calendar, timeline = self.detect_upcoming_expenses(
            user_uuid, account_id, days)
        df = run_hybrid_engine(S0, mu, params, days,
                               paths, account_id, calendar)
        return df, timeline
    ...
    def run_expense_simulation(self, account_id, E0, mu, user_uuid, days=30, paths=1000000, discipline_multiplier=1.0, drift_adjustment=0.0, stress_test_active=False, macro_environment="Stable", current_balance=None):
        ...
        if np.all(calendar == 0):
            logger.info(
                f"No upcoming bills detected for {account_id}. Returning zero expense projection.")
        ...
        if discipline_multiplier != 1.0 or drift_adjustment != 0.0 or stress_test_active or macro_environment != "Stable":
            ...
            if macro_environment == "Inflationary":
                ...
                mu -= 0.01
        df = run_converged_expense_engine(
            E0, mu, days, paths, account_id, calendar)
    ...
    def run_scenario_simulation(self, account_id, S0, mu, user_uuid, days=60, paths=1000000, injections=None):
        """
        Natively run a simulation with complex hypothetical injections.
        Injections is a list of dicts: [{'day': int, 'amount': float, 'type': 'one-off'|'recurring_monthly', 'description': str}]
        """
        ...
        for inj in injections:
            day = inj.get('day', 0)
            amount = inj.get('amount', 0.0)
            inj_type = inj.get('type', 'one-off')
            desc = inj.get('description', 'Scenario Event')

            if inj_type == 'one-off':
                if 0 <= day <= days:
                    calendar[day] += amount
                    timeline.append({'day': day, 'date': (pd.Timestamp.utcnow() + pd.Timedelta(days=day)).strftime(
                        '%Y-%m-%d'), 'merchant': f"[SCENARIO] {desc}", 'category': 'Scenario', 'amount': round(float(amount), 2)})
            elif inj_type == 'recurring_monthly':
                curr_day = day
                while curr_day <= days:
                    calendar[curr_day] += amount
                    timeline.append({'day': curr_day, 'date': (pd.Timestamp.utcnow() + pd.Timedelta(days=curr_day)).strftime(
                        '%Y-%m-%d'), 'merchant': f"[SCENARIO] {desc}", 'category': 'Scenario', 'amount': round(float(amount), 2)})
                    curr_day += 30

        timeline = sorted(timeline, key=lambda x: x['day'])
