...
class ForecasterAgent:
    ...
    def generate_dynamic_parameters(self, user_uuid):
        try:
            ...
            # Sequence Input: [Amount, CategoryID, Seasonal_Bias]
            feature_vector = np.array([amounts, category_ids, [context_avg]*30], dtype=np.float32).T # (30, 3)
            ...
            logger.info(f"LSTM Sample Input Data (First 3 days) - [Amount, CatID, SeasonalBias]:\n{feature_vector[:3]}")
            ...
            with SessionLocal() as session:
                ...
                session.commit()
                logger.info(f"LSTM Generated Bates Parameters for {user_uuid}: kappa={params.kappa:.4f}, theta={params.theta:.4f}, xi={params.xi:.4f}, lambda={params.lambda_val:.4f}")
    ...
    def detect_upcoming_expenses(self, user_uuid):
        user_acc = UserAccounts(user_id=user_uuid)
        df = user_acc.get_transactions("ALL", user_uuid)
        if df.empty:
            return []
    ...
    def run_hybrid_simulation(self, account_id, S0, mu, user_uuid, days=60, paths=1000, 
        ...
        if discipline_multiplier != 1.0 or drift_adjustment != 0.0 or stress_test_active or macro_environment != "Stable":
            logger.warning(f"SIMULATION OVERRIDES APPLIED for user {user_uuid} on account {account_id}")
            
            if discipline_multiplier < 1.0: # Strict
                params['theta'] *= 0.8
                params['xi'] *= 0.7
            elif discipline_multiplier > 1.0: # Erratic
                params['theta'] *= 1.4
                params['xi'] *= 1.5
