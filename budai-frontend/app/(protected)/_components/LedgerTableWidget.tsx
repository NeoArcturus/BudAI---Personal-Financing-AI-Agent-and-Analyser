...
export default function LedgerTableWidget({
  ...
  const { data: insight, isLoading: isAnalyzing } = useAdvisorInsight("ledgerTable", transactions.slice(0, 15));
  ...
  const getCategoryTheme = (category: string) => {
    const themes: Record<string, string> = {
      ...
      Groceries:
        "bg-[#FF8C00]/20 text-[#FF8C00] border-[#FF8C00]/40 shadow-[0_0_10px_rgba(255,140,0,0.15)]",
      Transportation:
      ...
      Rent:
        "bg-[#00BFFF]/20 text-[#00BFFF] border-[#00BFFF]/40 shadow-[0_0_10px_rgba(0,191,255,0.15)]",
      Shopping:
      ...
      "Health & Wellness":
        "bg-[#00FF7F]/20 text-[#00FF7F] border-[#00FF7F]/40 shadow-[0_0_10px_rgba(0,255,127,0.15)]",
      "Transfers & Investments":
      ...
      Salary:
        "bg-[#39FF14]/20 text-[#39FF14] border-[#39FF14]/40 shadow-[0_0_10px_rgba(57,255,20,0.2)]",
      Income:
        "bg-[#39FF14]/20 text-[#39FF14] border-[#39FF14]/40 shadow-[0_0_10px_rgba(57,255,20,0.15)]",
      Travel:
        "bg-[#FFD700]/20 text-[#FFD700] border-[#FFD700]/40 shadow-[0_0_10px_rgba(255,215,0,0.15)]",
      Education:
        "bg-[#ADFF2F]/20 text-[#ADFF2F] border-[#ADFF2F]/40 shadow-[0_0_10px_rgba(173,255,47,0.15)]",
    };
    return (
  ...
  return (
    ...
    >
      <Card className="w-full h-full obsidian-glass rounded-3xl shadow-2xl flex flex-col overflow-hidden relative font-geist">
          ...
          {isLoading ? (
            <div className="w-full h-full px-6 py-4 space-y-4 overflow-hidden">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                ...
                <Skeleton className="h-3 w-1/6 rounded bg-white/5" />
                <Skeleton className="h-3 w-1/6 rounded bg-white/5" />
                <Skeleton className="h-3 w-1/6 rounded bg-white/5" />
              </div>
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="flex items-center justify-between gap-4 py-2">
                  <div className="flex items-center gap-4 w-1/4 sm:w-[40%]">
                    <Skeleton className="w-9 h-9 rounded-full shrink-0 bg-white/5" />
                    <Skeleton className="h-4 w-full rounded bg-white/5 hidden sm:block" />
                  </div>
                  <Skeleton className="h-4 w-[30%] sm:w-[15%] rounded bg-white/5" />
                  <Skeleton className="h-4 w-[25%] sm:w-[15%] rounded bg-white/5" />
                  <Skeleton className="h-6 w-[25%] sm:w-[20%] rounded-md bg-white/5" />
                </div>
              ))}
            </div>
                        ...
                        >
                          <Table.Cell className="py-4 px-6 w-[20%] sm:w-[40%]">
                            <div className="flex items-center gap-4 w-full">
                              <Avatar
                                ...
                                className={
                                  isPositive
                                    ? "w-9 h-9 shrink-0 shadow-md rounded-full flex items-center justify-center border-2 border-[#39FF14] bg-[#39FF14]/40"
                                    : "w-9 h-9 shrink-0 shadow-md rounded-full flex items-center justify-center border-2 border-[#FF3366] bg-[#FF3366]/40"
                                }
                              >
                          ...
                          <Table.Cell className="py-4 px-6 w-[30%] sm:w-[15%]">
                            <span
                              className={cn(
                                "text-sm font-medium",
                                isPositive ? "text-[#39FF14]" : "text-[#FF3366]",
                              )}
                            >
                              {isPositive ? "+" : "-"} £{" "}
        ...
        >
          <Modal.Backdrop className="fixed inset-0 z-100 bg-black/60 backdrop-blur-sm">
            <Modal.Container className="fixed inset-0 z-101 flex items-center justify-center p-4">
              <Modal.Dialog className="bg-obsidian border border-white/8 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] p-6 relative max-w-md w-full pointer-events-auto backdrop-blur-xl">
