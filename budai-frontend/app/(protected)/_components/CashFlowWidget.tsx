...
export default function CashFlowWidget({ onRemove }: CashFlowWidgetProps) {
  ...
  return (
    ...
    >
      <Card className="w-full h-full obsidian-glass rounded-3xl p-6 flex flex-col shadow-2xl relative overflow-hidden font-geist">
        ...
        <Card.Content className="flex-1 w-full min-h-0 flex items-center justify-center p-0 z-0 [&::-webkit-scrollbar]:hidden">
          {isInitialLoading && transactions.length === 0 ? (
            <Skeleton className="w-full h-full rounded-3xl bg-white/5 mx-6" />
          ) : config ? (
            <div
              className={`w-full h-full ${isFetching ? "opacity-70 transition-opacity" : ""}`}
            >
